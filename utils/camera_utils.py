...
def loadCam(args, id, cam_info, resolution_scale):
    ...
    return Camera(
        ...
        fid=cam_info.fid if hasattr(cam_info, "fid") else None,
...
def get_intrinsic(fovx, fovy, w, h):
    ...
    cx = w / 2
    cy = h / 2
    intrinsic = np.array([[fx, 0.0, cx], [0.0, fy, cy], [0.0, 0.0, 1.0]]).astype(
        np.float32
    )
    return intrinsic
...
def transform_w2c(
    pw: torch.Tensor, R_w2c: np.ndarray, t_w2c: np.ndarray, intrinsic: np.ndarray
) -> torch.Tensor:
