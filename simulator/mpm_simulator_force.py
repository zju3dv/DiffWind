...
class CuboidModifier:
    def __init__(self):
        self.start_time = ti.field(dtype=ti.f32, shape=())
        self.end_time = ti.field(dtype=ti.f32, shape=())
        self.point = ti.Vector.field(3, dtype=ti.f32, shape=())
        self.size = ti.Vector.field(3, dtype=ti.f32, shape=())

    def set(self, start_time, end_time, point, size):
        self.start_time[None] = start_time
        self.end_time[None] = end_time
        self.point[None] = point
        self.size[None] = size


@ti.data_oriented
class MPMSimulator_Force:
    ...
    def __init__(
        ...
        **_,
        ...
        if self.material == MPMSimulator_Force.von_mises:
        ...
        self.step_particle = self.particle.dense(
            ti.j, cuda_chunk_size + 1
        ...
        self.C = ti.Matrix.field(
            dim, dim, dtype=self.dtype, needs_grad=True
        ...
        self.sig_out = ti.Vector.field(
            dim, dtype=self.dtype, needs_grad=True
        )  # Principal stretches for isotropic materials.
        self.force_field = ti.Vector.field(
            3, dtype=ti.f32, shape=(128, 128, 128), needs_grad=True
        )  # force field
        self.eps = 1e-7
        ...
        self.time = ti.field(self.dtype, shape=())
        self.time[None] = 0.0

        self.grid_velocity_modifiers = []

        self.force_modifier = CuboidModifier()
        self.static_part_modifier = CuboidModifier()

    @ti.kernel
    def from_torch_force_field(self, force_field: ti.types.ndarray()):
        for i, j, k in ti.ndrange(128, 128, 128):
            self.force_field[i, j, k] = ti.Vector(
                [
                    force_field[i, j, k, 0],
                    force_field[i, j, k, 1],
                    force_field[i, j, k, 2],
    ...
    def svd_grad(self):
        for p in range(self.n_particles[None]):
            self.F_tmp.grad[p] += self.backward_svd(
                self.U.grad[p].cast(ti.f64),
                self.sig.grad[p].cast(ti.f64),
                self.V.grad[p].cast(ti.f64),
                self.U[p].cast(ti.f64),
                self.sig[p].cast(ti.f64),
                self.V[p].cast(ti.f64),
            )
    ...
    def project_F(self, s: ti.i32):
        for p in range(self.n_particles[None]):
            sig = ti.Vector(
                [
                    ti.max(self.sig[p][0, 0], 0.05),
                    ti.max(self.sig[p][1, 1], 0.05),
                    ti.max(self.sig[p][2, 2], 0.05),
                ]
            )
            if ti.static(self.material == self.viscous_fluid):
                ...
                if shifted_trace >= 0:
                    ...
                    delta_gamma = (
                        epsilon_hat_norm
                        + (self.dim * self.lam[p] + 2.0 * self.mu[p])
                        / (2.0 * self.mu[p])
                        * (shifted_trace)
                        * self.friction_alpha[None]
                    )
                ...
                self.F[p, s + 1] = (
                    self.U[p]
                    @ self.make_matrix_from_diag(sig_out)
                    @ self.V[p].transpose()
                )
                ...
                if self.non_newtonian == 0:
                    ...
                    y = s_trial_norm - ti.sqrt(2.0 / 3) * self.yield_stress[None]

                    if y > 0:
                        mu_hat = self.mu[p] * b_trial.sum() / self.dim
                        s_new_norm = s_trial_norm - y / (
                            1
                            + self.plastic_viscosity[None]
                            / (2 * mu_hat * self.dt[None])
                        )
    ...
    def clamp(self, a, eps=1e-6):
        # Keep the denominator away from zero without an early return in Taichi.
        if a >= 0:
            a = ti.max(a, eps)
        else:
            a = ti.min(a, -eps)
    ...
    @ti.func
    def stencil_is_inside_grid(self, base):
        inside = True
        for axis in ti.static(range(3)):
            inside = inside and base[axis] >= self.offset[axis]
            inside = inside and base[axis] + 2 < self.offset[axis] + self.grid_size
        return inside
    ...
    def p2g(self, s: ti.i32):
        ...
        for p in range(self.n_particles[None]):
            ...
            if ti.static(self.material == self.elasticity):
                ...
                stress = (
                    self.damping_coeff * epsilon * J
                    + self.mu[p] * (new_F @ new_F.transpose())
                    + scale * ti.Matrix.identity(self.dtype, self.dim)
                )
            elif ti.static(self.material == self.viscous_fluid):
                J = new_F[0, 0]
                kappa = 0.6666666666 * self.mu[p] + self.lam[p]
                stress = (
                    kappa
                    * ti.Matrix.identity(self.dtype, self.dim)
                    * (J - 1 / (J**6))
                ...
                stress = (
                    self.U[p] @ self.make_matrix_from_diag(tau) @ self.U[p].transpose()
                )

            stress = (
                -self.dt[None] * self.p_vol[p] * 4 * self.inv_dx[None] ** 2
            ) * stress
            affine = stress + self.p_mass[p] * self.C[p, s]
            if self.stencil_is_inside_grid(base):
                    ...
                    for j in ti.static(range(3)):
                        for k in ti.static(range(3)):
                            ...
                            self.grid_v_in[base + offset] += weight * (
                                self.p_mass[p] * self.v[p, s] + affine @ dpos
                            )
    ...
    def grid_op(self, s: ti.i32):
        for I in ti.grouped(self.grid_m):
            if self.grid_m[I] > 1e-8 * self.dx[None] ** 3:
                v_out = (
                    self.grid_v_in[I] / self.grid_m[I]
                    + self.dt[None] * self.gravity[None]
                )
                for i in ti.static(range(len(self.analytic_collision))):
                    v_out = self.analytic_collision[i](I, v_out)

                for j in ti.static(range(len(self.grid_velocity_modifiers))):
                    v_out = self.grid_velocity_modifiers[j](
                        I, v_out, self.force_modifier
                    )
    ...
    def g2p(self, f: ti.i32):  # grid to particle (G2P)
        ...
        for p in range(self.n_particles[None]):
            ...
            if self.stencil_is_inside_grid(base):
                ...
                self.v[p, f + 1] = new_v
                self.x[p, f + 1] = self.x[p, f] + self.dt[None] * new_v
                self.C[p, f + 1] = new_C
            else:
                self.v[p, f + 1] = ti.Vector.zero(self.dtype, self.dim)
                self.x[p, f + 1] = self.x[p, f]
                self.C[p, f + 1] = ti.Matrix.zero(
                    self.dtype, self.dim, self.dim
                )
    ...
    def substep(self, s, cache=True):
        ...
        self.time[None] = self.time[None] + self.dt[None]
    ...
    def set_x(self, f: ti.i32, x: ti.types.ndarray()):
        ...
        for i in range(self.n_particles[None]):
            for d in ti.static(range(3)):
                self.x[i, local_index][d] = ti.cast(x[i, d], self.dtype)

    @ti.kernel
    def get_F(self, f: ti.i32, F: ti.types.ndarray()):
        local_index = (f * self.n_substeps[None]) % self.cuda_chunk_size
        for i in range(self.n_particles[None]):
            for d in ti.static(range(3)):
                for j in ti.static(range(3)):
                    F[i, d, j] = ti.cast(self.F[i, local_index][d, j], ti.f32)

    @ti.kernel
    def set_F(self, f: ti.i32, F: ti.types.ndarray()):
        local_index = (f * self.n_substeps[None]) % self.cuda_chunk_size
        for i in range(self.n_particles[None]):
            for d in ti.static(range(3)):
                for j in ti.static(range(3)):
                    self.F[i, local_index][d, j] = ti.cast(F[i, d, j], self.dtype)

    @ti.kernel
    def get_C(self, f: ti.i32, C: ti.types.ndarray()):
        local_index = (f * self.n_substeps[None]) % self.cuda_chunk_size
        for i in range(self.n_particles[None]):
            for d in ti.static(range(3)):
                for j in ti.static(range(3)):
                    C[i, d, j] = ti.cast(self.C[i, local_index][d, j], ti.f32)

    @ti.kernel
    def set_C(self, f: ti.i32, C: ti.types.ndarray()):
        local_index = (f * self.n_substeps[None]) % self.cuda_chunk_size
        for i in range(self.n_particles[None]):
            for d in ti.static(range(3)):
                for j in ti.static(range(3)):
                    self.C[i, local_index][d, j] = ti.cast(C[i, d, j], self.dtype)
    ...
    def set_v(self, f: ti.i32, v: ti.types.ndarray()):
        ...
        for i in range(self.n_particles[None]):
            for d in ti.static(range(3)):
                self.v[i, local_index][d] = ti.cast(v[i, d], self.dtype)
    ...
    def advance_grad(self, f):
        for i in reversed(
            range(self.n_substeps[None] * f, self.n_substeps[None] * (f + 1))
        ):
            if self.cfl_satisfy[None]:
                self.substep_grad(i)

    def add_cylinder_collider(self, start, end, radius, surface=surface_sticky):
    ...
    def add_force_field(
        ...
        self.force_modifier.set(
            start_time=start_time,
            end_time=start_time + self.frame_dt * num_frames,
            point=ti.Vector([point[0], point[1], point[2]]),
            size=ti.Vector([size[0], size[1], size[2]]),
        )

        @ti.func
        def apply_force(I, v, modifier):
            if (
                self.time[None] >= modifier.start_time[None]
                and self.time[None] < modifier.end_time[None]
            ):
                offset = I.cast(self.dtype) * self.dx[None] - modifier.point[None]
                if (
                    ti.abs(offset[0]) < modifier.size[None][0]
                    and ti.abs(offset[1]) < modifier.size[None][1]
                    and ti.abs(offset[2]) < modifier.size[None][2]
                ):
                    impulse = self.force_field[I] / self.grid_m[I]
                    v = v + impulse * self.dt[None]

            return v

        self.grid_velocity_modifiers.append(apply_force)

    def set_static_part_on_cuboid(
        self,
        point=(1, 1, 1),
        size=(1, 1, 1),
    ):

        self.static_part_modifier.set(
            start_time=0.0,
            end_time=0.0,
            point=ti.Vector([point[0], point[1], point[2]]),
            size=ti.Vector([size[0], size[1], size[2]]),
        ...
        def set_static_part(
            ...
            offset = (
                I.cast(self.dtype) * self.dx[None]
                - self.static_part_modifier.point[None]
            )
            if (
                ti.abs(offset[0]) < self.static_part_modifier.size[None][0]
                and ti.abs(offset[1]) < self.static_part_modifier.size[None][1]
                and ti.abs(offset[2]) < self.static_part_modifier.size[None][2]
            ):
                v = ti.Vector.zero(self.dtype, self.dim)
            return v

        self.grid_velocity_modifiers.append(set_static_part)
    ...
    def get_grid_v(self):
        return self.grid_v_in.to_numpy()
